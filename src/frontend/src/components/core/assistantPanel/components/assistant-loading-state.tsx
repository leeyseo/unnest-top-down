...
// it for a minimal branded loading indicator.
...
function AssistantLoadingIcon({ size = 24 }: { size?: number }) {
  return (
    <span
    ...
    >
      <UnnestLogo style={{ width: size, height: size }} />
    </span>
  );
}
...
function AssistantLoadingStateComponent({
  ...
  // bordered card (which would render essentially empty) for a branded
  // loading indicator. Must come AFTER all hook calls to preserve hook order.
  ...
  return (
    <div className="w-full max-w-[600px] py-1">
      {/* Header — status line, flat (no surrounding card). Uses the same
          AssistantLoadingIcon (size 24) as the flow-build minimal mode so the
